<x-layout.admin>
    <x-slot:title>
        Edit Service | Mirra Salon
    </x-slot:title>

    <x-admin.main>
        <x-admin.breadcrumb parent="services" child="edit"/>

        <div class="w-full mx-auto bg-white rounded-xl shadow-sm p-6">
            <h1 class="text-3xl font-semibold text-gray-800 mb-1">Edit Service</h1>
            <p class="text-gray-500 mb-6">Update the service information below.</p>
        
            <form action="/admin/services/{{ $service->id }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div class="space-y-4 text-gray-700">
                    ...
                    <div class="flex flex-col">
                        ...
                        @if($service->image_url)
                            <div class="mt-4">
                                ...
                                <div class="w-48 h-48 border rounded overflow-hidden">
                                    <img src="{{ asset('storage/' . $service->image_url) }}" alt="Image Service" class="object-cover w-full h-full">
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="number" id="time" name="time" min="1" required value="{{ $service->time }}"
                            class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400"

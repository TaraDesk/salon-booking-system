<x-layout.admin>
    <x-slot:title>
        Create Transaction | Mirra Salon
    </x-slot:title>

    <x-admin.main>
        <x-admin.breadcrumb parent="transactions" child="create"/>

        <div class="w-full mx-auto bg-white rounded-xl shadow-sm p-6">
            <h1 class="text-3xl font-semibold text-gray-800 mb-1">Create Transaction</h1>
            <p class="text-gray-500 mb-6">Enter Transaction information below.</p>
        
            <form action="/admin/transactions" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="space-y-4 text-gray-700">
                        ...
                        <select id="booking_id" name="booking_id" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400">
                            ...
                            @else
                                @foreach ($bookings as $booking)
                                    <option value="{{ $booking->id }}">Record #{{ $booking->id }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="date" id="payment_date" name="payment_date" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400">
                    </div>
